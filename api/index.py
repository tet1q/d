...
def catch_all(path):
    ...
    return jsonify({"status": "success", "message": "Stockfish API is running."})

@app.route('/get_best_move', methods=['POST'])
def get_best_move():
    if not engine: return jsonify({"error": "Stockfish engine is not available."}), 500
    data = request.json
    fen = data.get('fen')
    move_time = data.get('movetime', 2000) / 1000.0
    if not fen: return jsonify({"error": "FEN string is required."}), 400
    try:
        board = chess.Board(fen)
        result = engine.play(board, chess.engine.Limit(time=move_time))
        best_move = result.move
        return jsonify({"fen_before": fen, "best_move_uci": best_move.uci(), "best_move_san": board.san(best_move), "thinking_time_ms": move_time * 1000})
...
def evaluate_position():
    ...
    try:
        ...
        return jsonify({"fen": fen, "evaluation": {"type": score.score_type(), "value": score.score(), "mate_in": score.mate()}, "turn": "white" if board.turn == chess.WHITE else "black"})
    except Exception as e: return jsonify({"error": str(e)}), 500

if __name__ == '__main__':
    if not engine: print(f"GAGAL MEMUAT ENGINE: {engine_error}")
